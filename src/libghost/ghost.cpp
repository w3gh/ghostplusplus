...
CGame *CGHost :: GetCurrentGame() const
{
	return m_CurrentGame;
}

CAdminGame *CGHost :: GetAdminGame() const
{
	return m_AdminGame;
}

CLanguage *CGHost :: GetLanguage( ) const
{
	return m_Language;
}

CMap *CGHost :: GetCurrentMap( ) const
{
	return m_Map;
}

void CGHost :: SetCurrentMap( CMap *map)
{
	if( m_Map )
		delete m_Map;
	m_Map = map;
...
CMap *CGHost :: GetAdminMap( ) const
{
	return m_AdminMap;
}

CMap *CGHost :: GetAutoHostMap( ) const
{
	return m_AutoHostMap;
}

void CGHost :: SetAutoHostMap( CMap *map)
{
	if( m_AutoHostMap )
		...
		m_ConfigFile(configFile)
{
	...
	for( quint32 i = 1; i < 10; i++ )
	{
		...
		QObject::connect(bnet,
						 SIGNAL(SignalBnetCommand(CBNET*, const QString&, const QString&, const QString&, bool)),
						 this,
						 SLOT(EventBnetCommand(CBNET*, const QString&, const QString&, const QString&, bool)));
		m_BNETs.push_back( bnet );
	}
	...
	if( m_AdminGameCreate )
	{
		...
		QObject::connect(m_AdminGame,
						 SIGNAL(SignalPlayerCommand(CBaseGame*, CGamePlayer *, const QString &, const QString &)),
						 this,
						 SLOT(EventGameCommand(CBaseGame*, CGamePlayer *, const QString &, const QString &)));
...
void CGHost :: LoadPlugin( QObject *plugin, CConfig *cfg )
{
