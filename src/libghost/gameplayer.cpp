		...
		for( QList<CBNET *> :: const_iterator i = m_Game->m_GHost->m_BNETs.begin( ); i != m_Game->m_GHost->m_BNETs.end( ); i++ )
		{
			if( (*i)->GetServer( ) == m_JoinedRealm )
			{
				if( m_Game->GetGameState( ) == GAME_PUBLIC )
				{
					...
					(*i)->QueueChatCommand( m_Game->m_GHost->GetLanguage( )->SpoofCheckByReplying( ), m_Name, true );
			}
		}
...
void CPotentialPlayer :: ExtractPackets( )
{
	...
	while( m_Socket->bytesAvailable() >= 4 )
	{
		...
		quint16 Length = Util::extractUInt16( m_Socket->peek(4), 2 );
...
void CGamePlayer::EventSpoofCheckTimeout()
{
	...
	deleteLater();
	SetLeftReason( m_Game->m_GHost->GetLanguage( )->WasKickedForNotSpoofChecking( ) );
...
void CGamePlayer::EventSendGProxyMessage()
{
	...
	m_Game->SendAllChat( GetPID( ), m_Game->m_GHost->GetLanguage( )->WaitForReconnectSecondsRemain( QString::number( TimeRemaining ) ) );
	...
	while( !m_Packets.isEmpty( ) )
	{
		...
		if( Packet->GetPacketType( ) == W3GS_HEADER_CONSTANT )
		{
			...
			case CGameProtocol :: W3GS_MAPSIZE:
				MapSize = m_Protocol->RECEIVE_W3GS_MAPSIZE( Packet->GetData( ), m_Game->m_GHost->GetCurrentMap( )->GetMapSize( ) );
		...
		else if( Packet->GetPacketType( ) == GPS_HEADER_CONSTANT )
		{
			...
			{
				int LastPacket = Util::extractUInt32( Data, 4 );
...
void CGamePlayer :: EventGProxyReconnect( QTcpSocket *NewSocket, quint32 LastPacket )
{
	...
	m_GProxyDisconnectNoticeSent = false;
	m_Game->SendAllChat( m_Game->m_GHost->GetLanguage( )->PlayerReconnectedWithGProxy( m_Name ) );
}
