...
void CGame::EventCallableUpdateTimeout()
{
	...
	for( QList<PairedBanCheck> :: iterator i = m_PairedBanChecks.begin( ); i != m_PairedBanChecks.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			if( Ban )
				SendAllChat( m_GHost->GetLanguage( )->UserWasBannedOnByBecause( i->second->GetServer( ), i->second->GetUser( ), Ban->GetDate( ), Ban->GetAdmin( ), Ban->GetReason( ) ) );
			else
				SendAllChat( m_GHost->GetLanguage( )->UserIsNotBanned( i->second->GetServer( ), i->second->GetUser( ) ) );
	...
	for( QList<PairedBanAdd> :: iterator i = m_PairedBanAdds.begin( ); i != m_PairedBanAdds.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			if( i->second->GetResult( ) )
			{
				...
				SendAllChat( m_GHost->GetLanguage( )->PlayerWasBannedByPlayer( i->second->GetServer( ), i->second->GetUser( ), i->first ) );
			}
	...
	for( QList<PairedGPSCheck> :: iterator i = m_PairedGPSChecks.begin( ); i != m_PairedGPSChecks.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			{
				if( i->first.isEmpty( ) )
					SendAllChat( m_GHost->GetLanguage( )->HasntPlayedGamesWithThisBot( i->second->GetName( ) ) );
				else
				{
					CGamePlayer *Player = GetPlayerFromName( i->first, true );

					if( Player )
						SendChat( Player, m_GHost->GetLanguage( )->HasntPlayedGamesWithThisBot( i->second->GetName( ) ) );
				}
			}
	...
	for( QList<PairedDPSCheck> :: iterator i = m_PairedDPSChecks.begin( ); i != m_PairedDPSChecks.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			{
				if( i->first.isEmpty( ) )
					SendAllChat( m_GHost->GetLanguage( )->HasntPlayedDotAGamesWithThisBot( i->second->GetName( ) ) );
				else
				{
					CGamePlayer *Player = GetPlayerFromName( i->first, true );

					if( Player )
						SendChat( Player, m_GHost->GetLanguage( )->HasntPlayedDotAGamesWithThisBot( i->second->GetName( ) ) );
				}
			}
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, QString command, QString payload )
{
	...
	if( player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || IsOwner( User ) ) )
	{
			...
			******************/
			...
			if( ( Command == "addban" || Command == "ban" ) && !Payload.isEmpty( ) && !m_GHost->m_BNETs.isEmpty( ) )
			{
				...
				if( m_GameLoaded )
				{
					...
					if( Matches == 0 )
						SendAllChat( m_GHost->GetLanguage( )->UnableToBanNoMatchesFound( Victim ) );
					else if( Matches == 1 )
						...
						SendAllChat( m_GHost->GetLanguage( )->UnableToBanFoundMoreThanOneMatch( Victim ) );
				}
				else
				{
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							SendAllChat( m_GHost->GetLanguage( )->AnnounceMessageEnabled( ) );
			...
			if( Command == "autosave" )
			{
				if( Payload == "on" )
				{
					SendAllChat( m_GHost->GetLanguage( )->AutoSaveEnabled( ) );
					m_AutoSave = true;
				}
				else if( Payload == "off" )
				{
					SendAllChat( m_GHost->GetLanguage( )->AutoSaveDisabled( ) );
					m_AutoSave = false;
				}
			}
			...
			if( Command == "autostart" && !m_CountDownStarted )
			{
				if( Payload.isEmpty( ) || Payload == "off" )
				{
					SendAllChat( m_GHost->GetLanguage( )->AutoStartDisabled( ) );
				...
				else
				{
					...
					{
						SendAllChat( m_GHost->GetLanguage( )->AutoStartEnabled( QString::number( AutoStartPlayers ) ) );
			...
			if( Command == "check" )
			{
				if( !Payload.isEmpty( ) )
				{
					...
					if( Matches == 0 )
						SendAllChat( m_GHost->GetLanguage( )->UnableToCheckPlayerNoMatchesFound( Payload ) );
					else if( Matches == 1 )
					{
			...
			if( Command == "clearhcl" && !m_CountDownStarted )
			{
				...
				SendAllChat( m_GHost->GetLanguage( )->ClearingHCL( ) );
			}
			...
			if( ( Command == "download" || Command == "dl" ) && !Payload.isEmpty( ) && !m_GameLoading && !m_GameLoaded )
			{
				...
				if( Matches == 0 )
					SendAllChat( m_GHost->GetLanguage( )->UnableToStartDownloadNoMatchesFound( Payload ) );
				else if( Matches == 1 )
				{
					...
					SendAllChat( m_GHost->GetLanguage( )->UnableToStartDownloadFoundMoreThanOneMatch( Payload ) );
			}
				...
				if( !Payload.isEmpty( ) )
				{
					if( Payload.size( ) <= m_Slots.size( ) )
					{
						...
						if( Payload.indexOf( QRegExp("[^" + HCLChars + "]") ) == -1 )
						{
							...
							SendAllChat( m_GHost->GetLanguage( )->SettingHCL( m_HCLCommandString ) );
						}
						else
							SendAllChat( m_GHost->GetLanguage( )->UnableToSetHCLInvalid( ) );
					}
					else
						SendAllChat( m_GHost->GetLanguage( )->UnableToSetHCLTooLong( ) );
				}
				else
					SendAllChat( m_GHost->GetLanguage( )->TheHCLIs( m_HCLCommandString ) );
			}
			...
			if( Command == "hold" && !Payload.isEmpty( ) && !m_GameLoading && !m_GameLoaded )
			{
				...
				while( !SS.atEnd( ) )
				{
					...
					{
						SendAllChat( m_GHost->GetLanguage( )->AddedPlayerToTheHoldList( HoldName ) );
			...
			if( Command == "kick" && !Payload.isEmpty( ) )
			{
				...
				if( Matches == 0 )
					SendAllChat( m_GHost->GetLanguage( )->UnableToKickNoMatchesFound( Payload ) );
				else if( Matches == 1 )
				{
					LastMatch->SetLeftReason( m_GHost->GetLanguage( )->WasKickedByPlayer( User ) );
					...
					SendAllChat( m_GHost->GetLanguage( )->UnableToKickFoundMoreThanOneMatch( Payload ) );
			}
			...
			{
				if( Payload.isEmpty( ) )
					SendAllChat( m_GHost->GetLanguage( )->LatencyIs( QString::number( m_Latency ) ) );
				else
				{
					...
					if( m_RequestedLatency <= min )
					{
						...
						SendAllChat( m_GHost->GetLanguage( )->SettingLatencyToMinimum( QString::number(m_RequestedLatency) ) );
					}
					else if( m_RequestedLatency >= max )
					{
						m_RequestedLatency = max;
						SendAllChat( m_GHost->GetLanguage( )->SettingLatencyToMaximum( QString::number(m_RequestedLatency) ) );
					}
					else
						SendAllChat( m_GHost->GetLanguage( )->SettingLatencyTo( QString::number( m_RequestedLatency ) ) );
				}
			}
			...
			if( Command == "messages" )
			{
				if( Payload == "on" )
				{
					SendAllChat( m_GHost->GetLanguage( )->LocalAdminMessagesEnabled( ) );
					m_LocalAdminMessages = true;
				}
				else if( Payload == "off" )
				{
					SendAllChat( m_GHost->GetLanguage( )->LocalAdminMessagesDisabled( ) );
			...
			if( Command == "mute" )
			{
				...
				if( Matches == 0 )
					SendAllChat( m_GHost->GetLanguage( )->UnableToMuteNoMatchesFound( Payload ) );
				else if( Matches == 1 )
				{
					SendAllChat( m_GHost->GetLanguage( )->MutedPlayer( LastMatch->GetName( ), User ) );
					LastMatch->SetMuted( true );
				}
				else
					SendAllChat( m_GHost->GetLanguage( )->UnableToMuteFoundMoreThanOneMatch( Payload ) );
			}
			...
			{
				SendAllChat( m_GHost->GetLanguage( )->GlobalChatMuted( ) );
				m_MuteAll = true;
			}
			...
			if( Command == "owner" )
			{
				if( RootAdminCheck || IsOwner( User ) || !GetPlayerFromName( m_OwnerName, false ) )
				{
					if( !Payload.isEmpty( ) )
					{
						SendAllChat( m_GHost->GetLanguage( )->SettingGameOwnerTo( Payload ) );
						m_OwnerName = Payload;
					}
					else
					{
						SendAllChat( m_GHost->GetLanguage( )->SettingGameOwnerTo( User ) );
						m_OwnerName = User;
					}
				}
				else
					SendAllChat( m_GHost->GetLanguage( )->UnableToSetGameOwner( m_OwnerName ) );
			}
			...
			if( Command == "ping" )
			{
				...
				if( Kicked > 0 )
					SendAllChat( m_GHost->GetLanguage( )->KickingPlayersWithPingsGreaterThan( QString::number( Kicked ), QString::number( KickPing ) ) );
			}
			...
			if( Command == "priv" && !Payload.isEmpty( ) && !m_CountDownStarted && !m_SaveGame )
			{
				...
				SendAllChat( m_GHost->GetLanguage( )->TryingToRehostAsPrivateGame( Payload ) );
			...
			if( Command == "pub" && !Payload.isEmpty( ) && !m_CountDownStarted && !m_SaveGame )
			{
				...
				SendAllChat( m_GHost->GetLanguage( )->TryingToRehostAsPublicGame( Payload ) );
			...
			if( Command == "refresh" && !m_CountDownStarted )
			{
				if( Payload == "on" )
				{
					SendAllChat( m_GHost->GetLanguage( )->RefreshMessagesEnabled( ) );
					m_RefreshMessages = true;
				}
				else if( Payload == "off" )
				{
					SendAllChat( m_GHost->GetLanguage( )->RefreshMessagesDisabled( ) );
			...
			if( Command == "start" && !m_CountDownStarted )
			{
				...
				else
				{
					if( GetTicks( ) - m_LastPlayerLeaveTicks >= 2000 )
						...
						SendAllChat( m_GHost->GetLanguage( )->CountDownAbortedSomeoneLeftRecently( ) );
				}
			}
			...
			{
				if( Payload.isEmpty( ) )
					SendAllChat( m_GHost->GetLanguage( )->SyncLimitIs( QString::number( m_SyncLimit ) ) );
				else
				{
					...
					{
						m_SyncLimit = 10;
						SendAllChat( m_GHost->GetLanguage( )->SettingSyncLimitToMinimum( "10" ) );
					}
					else if( m_SyncLimit >= 10000 )
					{
						m_SyncLimit = 10000;
						SendAllChat( m_GHost->GetLanguage( )->SettingSyncLimitToMaximum( "10000" ) );
					}
					else
						SendAllChat( m_GHost->GetLanguage( )->SettingSyncLimitTo( QString::number( m_SyncLimit ) ) );
				}
			}
			...
			if( Command == "unmute" )
			{
				...
				{
					SendAllChat( m_GHost->GetLanguage( )->UnmutedPlayer( LastMatch->GetName( ), User ) );
			...
			{
				SendAllChat( m_GHost->GetLanguage( )->GlobalChatUnmuted( ) );
				m_MuteAll = false;
			}
		...
		else
		{
			...
			SendChat( player, m_GHost->GetLanguage( )->TheGameIsLocked( ) );
		}
	}
	else
	{
	...
	*********************/
	...
	if( Command == "checkme" )
		SendChat( player, m_GHost->GetLanguage( )->CheckedPlayer( User, player->GetNumPings( ) > 0 ? QString::number( player->GetPing( m_GHost->m_LCPings ) ) + "ms" : "N/A", m_GHost->m_DBLocal->FromCheck( Util::extractUInt32(Util::reverse(player->GetExternalIP( ))) ), AdminCheck || RootAdminCheck ? "Yes" : "No", IsOwner( User ) ? "Yes" : "No", player->GetSpoofed( ) ? "Yes" : "No", player->GetSpoofedRealm( ).isEmpty( ) ? "N/A" : player->GetSpoofedRealm( ), player->GetReserved( ) ? "Yes" : "No" ) );
	...
	if( Command == "votekick" && m_GHost->m_VoteKickAllowed && !Payload.isEmpty( ) )
	{
		if( !m_KickVotePlayer.isEmpty( ) )
			SendChat( player, m_GHost->GetLanguage( )->UnableToVoteKickAlreadyInProgress( ) );
		else if( m_Players.size( ) == 2 )
			SendChat( player, m_GHost->GetLanguage( )->UnableToVoteKickNotEnoughPlayers( ) );
		else
		{
			CGamePlayer *LastMatch = NULL;
			quint32 Matches = GetPlayerFromNamePartial( Payload, &LastMatch );
			...
			if( Matches == 0 )
				SendChat( player, m_GHost->GetLanguage( )->UnableToVoteKickNoMatchesFound( Payload ) );
			else if( Matches == 1 )
			{
				if( LastMatch->GetReserved( ) )
					SendChat( player, m_GHost->GetLanguage( )->UnableToVoteKickPlayerIsReserved( LastMatch->GetName( ) ) );
				else
				{
					...
					SendAllChat( m_GHost->GetLanguage( )->StartedVoteKick( LastMatch->GetName( ), User, QString::number( (quint32)ceil( ( GetNumHumanPlayers( ) - 1 ) * (float)m_GHost->m_VoteKickPercentage / 100 ) - 1 ) ) );
					SendAllChat( m_GHost->GetLanguage( )->TypeYesToVote( QString( 1, m_GHost->m_CommandTrigger ) ) );
				}
			}
			else
				SendChat( player, m_GHost->GetLanguage( )->UnableToVoteKickFoundMoreThanOneMatch( Payload ) );
		}
	}
	...
	if( Command == "yes" && !m_KickVotePlayer.isEmpty( ) && player->GetName( ) != m_KickVotePlayer && !player->GetKickVote( ) )
	{
		...
		if( Votes >= VotesNeeded )
		{
			...
			if( Victim )
			{
				Victim->SetLeftReason( m_GHost->GetLanguage( )->WasKickedByVote( ) );
				...
				SendAllChat( m_GHost->GetLanguage( )->VoteKickPassed( m_KickVotePlayer ) );
			}
			else
				SendAllChat( m_GHost->GetLanguage( )->ErrorVoteKickingPlayer( m_KickVotePlayer ) );

			m_KickVotePlayer.clear( );
			m_VotekickTimer.stop();
		}
		else
			SendAllChat( m_GHost->GetLanguage( )->VoteKickAcceptedNeedMoreVotes( m_KickVotePlayer, User, QString::number( VotesNeeded - Votes ) ) );
	}
