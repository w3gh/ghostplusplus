...
void CBNET::EventCallableUpdateTimeout()
{
	...
	for( QList<PairedAdminCount> :: iterator i = m_PairedAdminCounts.begin( ); i != m_PairedAdminCounts.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			if( Count == 0 )
				QueueChatCommand( m_GHost->GetLanguage( )->ThereAreNoAdmins( m_Server ), i->first, !i->first.isEmpty( ) );
			else if( Count == 1 )
				QueueChatCommand( m_GHost->GetLanguage( )->ThereIsAdmin( m_Server ), i->first, !i->first.isEmpty( ) );
			else
				QueueChatCommand( m_GHost->GetLanguage( )->ThereAreAdmins( m_Server, QString::number( Count ) ), i->first, !i->first.isEmpty( ) );
	...
	for( QList<PairedAdminAdd> :: iterator i = m_PairedAdminAdds.begin( ); i != m_PairedAdminAdds.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			if( i->second->GetResult( ) )
			{
				...
				QueueChatCommand( m_GHost->GetLanguage( )->AddedUserToAdminDatabase( m_Server, i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
			}
			else
				QueueChatCommand( m_GHost->GetLanguage( )->ErrorAddingUserToAdminDatabase( m_Server, i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
	...
	for( QList<PairedAdminRemove> :: iterator i = m_PairedAdminRemoves.begin( ); i != m_PairedAdminRemoves.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			if( i->second->GetResult( ) )
			{
				...
				QueueChatCommand( m_GHost->GetLanguage( )->DeletedUserFromAdminDatabase( m_Server, i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
			}
			else
				QueueChatCommand( m_GHost->GetLanguage( )->ErrorDeletingUserFromAdminDatabase( m_Server, i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
	...
	for( QList<PairedBanCount> :: iterator i = m_PairedBanCounts.begin( ); i != m_PairedBanCounts.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			if( Count == 0 )
				QueueChatCommand( m_GHost->GetLanguage( )->ThereAreNoBannedUsers( m_Server ), i->first, !i->first.isEmpty( ) );
			else if( Count == 1 )
				QueueChatCommand( m_GHost->GetLanguage( )->ThereIsBannedUser( m_Server ), i->first, !i->first.isEmpty( ) );
			else
				QueueChatCommand( m_GHost->GetLanguage( )->ThereAreBannedUsers( m_Server, QString::number( Count ) ), i->first, !i->first.isEmpty( ) );
	...
	for( QList<PairedBanAdd> :: iterator i = m_PairedBanAdds.begin( ); i != m_PairedBanAdds.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			if( i->second->GetResult( ) )
			{
				...
				QueueChatCommand( m_GHost->GetLanguage( )->BannedUser( i->second->GetServer( ), i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
			}
			else
				QueueChatCommand( m_GHost->GetLanguage( )->ErrorBanningUser( i->second->GetServer( ), i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
	...
	for( QList<PairedBanRemove> :: iterator i = m_PairedBanRemoves.begin( ); i != m_PairedBanRemoves.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			if( i->second->GetResult( ) )
			{
				...
				QueueChatCommand( m_GHost->GetLanguage( )->UnbannedUser( i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
			}
			else
				QueueChatCommand( m_GHost->GetLanguage( )->ErrorUnbanningUser( i->second->GetUser( ) ), i->first, !i->first.isEmpty( ) );
	...
	for( QList<PairedGPSCheck> :: iterator i = m_PairedGPSChecks.begin( ); i != m_PairedGPSChecks.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			if( GamePlayerSummary )
				QueueChatCommand( m_GHost->GetLanguage( )->HasPlayedGamesWithThisBot( i->second->GetName( ), GamePlayerSummary->GetFirstGameDateTime( ), GamePlayerSummary->GetLastGameDateTime( ), QString::number( GamePlayerSummary->GetTotalGames( ) ), QString::number( (float)GamePlayerSummary->GetAvgLoadingTime( ) / 1000, 'g', 2 ), QString::number( GamePlayerSummary->GetAvgLeftPercent( ) ) ), i->first, !i->first.isEmpty( ) );
			else
				QueueChatCommand( m_GHost->GetLanguage( )->HasntPlayedGamesWithThisBot( i->second->GetName( ) ), i->first, !i->first.isEmpty( ) );
	...
	for( QList<PairedDPSCheck> :: iterator i = m_PairedDPSChecks.begin( ); i != m_PairedDPSChecks.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			{
				QString Summary = m_GHost->GetLanguage( )->HasPlayedDotAGamesWithThisBot(	i->second->GetName( ),
				...
				QueueChatCommand( m_GHost->GetLanguage( )->HasntPlayedDotAGamesWithThisBot( i->second->GetName( ) ), i->first, !i->first.isEmpty( ) );
...
void CBNET :: ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	if( Event == CBNETProtocol :: EID_WHISPER || Event == CBNETProtocol :: EID_TALK )
	{
		...
		if( Event == CBNETProtocol :: EID_WHISPER && m_GHost->GetCurrentGame( ) )
		{
			if( Message == "s" || Message == "sc" || Message == "spoof" || Message == "check" || Message == "spoofcheck" )
				m_GHost->GetCurrentGame( )->AddToSpoofed( m_Server, User, true );
			else if( Message.indexOf( m_GHost->GetCurrentGame( )->GetGameName( ) ) != -1 )
			{
				...
				if( m_PasswordHashType == "pvpgn" && User == m_PVPGNRealmName )
				{
					...
					if( Tokens.size( ) >= 3 )
						m_GHost->GetCurrentGame( )->AddToSpoofed( m_Server, Tokens[2], false );
				}
				else
					m_GHost->GetCurrentGame( )->AddToSpoofed( m_Server, User, false );
			}
		}
		...
		if( Message == "?trigger" && ( IsAdmin( User ) || IsRootAdmin( User ) || ( m_PublicCommands && m_OutPackets.size( ) <= 3 ) ) )
			QueueChatCommand( m_GHost->GetLanguage( )->CommandTrigger( QString( 1, m_CommandTrigger ) ), User, Whisper );
		else if( !Message.isEmpty( ) && Message[0] == m_CommandTrigger )
		{
			...
			emit SignalBnetCommand( this, User, Command, Payload, Whisper );
			...
			if( IsAdmin( User ) || IsRootAdmin( User ) )
			{
				...
				******************/
				...
				if( Command == "addadmin" && !Payload.isEmpty( ) )
				{
					...
					{
						if( IsAdmin( Payload ) )
							QueueChatCommand( m_GHost->GetLanguage( )->UserIsAlreadyAnAdmin( m_Server, Payload ), User, Whisper );
						else
				...
				if( ( Command == "addban" || Command == "ban" ) && !Payload.isEmpty( ) )
				{
					...
					if( IsBannedName( Victim ) )
						QueueChatCommand( m_GHost->GetLanguage( )->UserIsAlreadyBanned( m_Server, Victim ), User, Whisper );
					else
				...
				if( Command == "announce" && m_GHost->GetCurrentGame( ) && !m_GHost->GetCurrentGame( )->GetCountDownStarted( ) )
				{
					if( Payload.isEmpty( ) || Payload == "off" )
					{
						QueueChatCommand( m_GHost->GetLanguage( )->AnnounceMessageDisabled( ), User, Whisper );
						m_GHost->GetCurrentGame( )->SetAnnounce( 0, QString( ) );
					}
					else
					{
						...
						else
						{
							...
							else
							{
								...
								QueueChatCommand( m_GHost->GetLanguage( )->AnnounceMessageEnabled( ), User, Whisper );
								m_GHost->GetCurrentGame( )->SetAnnounce( Interval, Message );
							}
						}
					}
				}
				...
				if( Command == "autohost" )
				{
					if( IsRootAdmin( User ) )
					{
						if( Payload.isEmpty( ) || Payload == "off" )
						{
							QueueChatCommand( m_GHost->GetLanguage( )->AutoHostDisabled( ), User, Whisper );
							...
							else
							{
								...
								else
								{
									...
									else
									{
										...
										QueueChatCommand( m_GHost->GetLanguage( )->AutoHostEnabled( ), User, Whisper );
				...
				if( Command == "autostart" && m_GHost->GetCurrentGame( ) && !m_GHost->GetCurrentGame( )->GetCountDownStarted( ) )
				{
					if( Payload.isEmpty( ) || Payload == "off" )
					{
						QueueChatCommand( m_GHost->GetLanguage( )->AutoStartDisabled( ), User, Whisper );
						m_GHost->GetCurrentGame( )->SetAutoStartPlayers( 0 );
					}
					else
					{
						...
						{
							QueueChatCommand( m_GHost->GetLanguage( )->AutoStartEnabled( QString::number( AutoStartPlayers ) ), User, Whisper );
							m_GHost->GetCurrentGame( )->SetAutoStartPlayers( AutoStartPlayers );
						}
					}
				}
				...
				if( Command == "checkadmin" && !Payload.isEmpty( ) )
				{
					...
					{
						if( IsAdmin( Payload ) )
							QueueChatCommand( m_GHost->GetLanguage( )->UserIsAnAdmin( m_Server, Payload ), User, Whisper );
						else
							QueueChatCommand( m_GHost->GetLanguage( )->UserIsNotAnAdmin( m_Server, Payload ), User, Whisper );
					}
					else
						QueueChatCommand( m_GHost->GetLanguage( )->YouDontHaveAccessToThatCommand( ), User, Whisper );
				}
				...
				if( Command == "checkban" && !Payload.isEmpty( ) )
				{
					...
					if( Ban )
						QueueChatCommand( m_GHost->GetLanguage( )->UserWasBannedOnByBecause( m_Server, Payload, Ban->GetDate( ), Ban->GetAdmin( ), Ban->GetReason( ) ), User, Whisper );
					else
						QueueChatCommand( m_GHost->GetLanguage( )->UserIsNotBanned( m_Server, Payload ), User, Whisper );
				}
				...
				if( Command == "close" && !Payload.isEmpty( ) && m_GHost->GetCurrentGame( ) )
				{
					if( !m_GHost->GetCurrentGame( )->GetLocked( ) )
					{
						...
						QueueChatCommand( m_GHost->GetLanguage( )->TheGameIsLockedBNET( ), User, Whisper );
				}
				...
				if( Command == "closeall" && m_GHost->GetCurrentGame( ) )
				{
					if( !m_GHost->GetCurrentGame( )->GetLocked( ) )
						m_GHost->GetCurrentGame( )->CloseAllSlots( );
					else
						QueueChatCommand( m_GHost->GetLanguage( )->TheGameIsLockedBNET( ), User, Whisper );
				}
				...
				if( Command == "downloads" && !Payload.isEmpty( ) )
				{
					...
					if( Downloads == 0 )
					{
						QueueChatCommand( m_GHost->GetLanguage( )->MapDownloadsDisabled( ), User, Whisper );
						m_GHost->m_AllowDownloads = 0;
					}
					else if( Downloads == 1 )
					{
						QueueChatCommand( m_GHost->GetLanguage( )->MapDownloadsEnabled( ), User, Whisper );
						m_GHost->m_AllowDownloads = 1;
					}
					else if( Downloads == 2 )
					{
						QueueChatCommand( m_GHost->GetLanguage( )->MapDownloadsConditional( ), User, Whisper );
				...
				if( Command == "end" && !Payload.isEmpty( ) )
				{
					...
					if( GameNumber < m_GHost->m_Games.size( ) )
					{
						...
						if( m_GHost->m_Games[GameNumber]->GetPlayerFromName( m_GHost->m_Games[GameNumber]->GetOwnerName( ), false ) && !IsRootAdmin( User ) )
							QueueChatCommand( m_GHost->GetLanguage( )->CantEndGameOwnerIsStillPlaying( m_GHost->m_Games[GameNumber]->GetOwnerName( ) ), User, Whisper );
						else
						{
							QueueChatCommand( m_GHost->GetLanguage( )->EndingGame( m_GHost->m_Games[GameNumber]->GetDescription( ) ), User, Whisper );
						...
						QueueChatCommand( m_GHost->GetLanguage( )->GameNumberDoesntExist( Payload ), User, Whisper );
				}
				...
				if( Command == "enforcesg" && !Payload.isEmpty( ) )
				{
					...
					if( Payload.indexOf( "/" ) != -1 || Payload.indexOf( "\\" ) != -1 )
						QueueChatCommand( m_GHost->GetLanguage( )->UnableToLoadReplaysOutside( ), User, Whisper );
					else
					{
						...
						{
							QueueChatCommand( m_GHost->GetLanguage( )->LoadingReplay( File ), User, Whisper );
							...
							QueueChatCommand( m_GHost->GetLanguage( )->UnableToLoadReplayDoesntExist( File ), User, Whisper );
					}
				}
					...
					if( IsRootAdmin( User ) )
					{
						...
						else
						{
							if( m_GHost->GetCurrentGame( ) || !m_GHost->m_Games.isEmpty( ) )
								QueueChatCommand( m_GHost->GetLanguage( )->AtLeastOneGameActiveUseForceToShutdown( ), User, Whisper );
							else
								m_GHost->deleteLater();
						}
					}
				...
				{
					SendGetClanList( );
					QueueChatCommand( m_GHost->GetLanguage( )->UpdatingClanList( ), User, Whisper );
				}
				...
				{
					SendGetFriendsList( );
					QueueChatCommand( m_GHost->GetLanguage( )->UpdatingFriendsList( ), User, Whisper );
				}
				...
				if( Command == "getgame" && !Payload.isEmpty( ) )
				{
					...
					if( GameNumber < m_GHost->m_Games.size( ) )
						QueueChatCommand( m_GHost->GetLanguage( )->GameNumberIs( Payload, m_GHost->m_Games[GameNumber]->GetDescription( ) ), User, Whisper );
					else
						QueueChatCommand( m_GHost->GetLanguage( )->GameNumberDoesntExist( Payload ), User, Whisper );
				}
				...
				if( Command == "getgames" )
				{
					if( m_GHost->GetCurrentGame( ) )
						QueueChatCommand( m_GHost->GetLanguage( )->GameIsInTheLobby( m_GHost->GetCurrentGame( )->GetDescription( ), QString::number( m_GHost->m_Games.size( ) ), QString::number( m_GHost->m_MaxGames ) ), User, Whisper );
					else
						QueueChatCommand( m_GHost->GetLanguage( )->ThereIsNoGameInTheLobby( QString::number( m_GHost->m_Games.size( ) ), QString::number( m_GHost->m_MaxGames ) ), User, Whisper );
				}
				...
				if( Command == "hold" && !Payload.isEmpty( ) && m_GHost->GetCurrentGame( ) )
				{
					...
					while( !SS.atEnd( ) )
					{
						...
						{
							QueueChatCommand( m_GHost->GetLanguage( )->AddedPlayerToTheHoldList( HoldName ), User, Whisper );
							m_GHost->GetCurrentGame( )->AddToReserved( HoldName );
						}
					}
				}
				...
				if( Command == "hostsg" && !Payload.isEmpty( ) )
					m_GHost->CreateGame( m_GHost->GetCurrentMap( ), GAME_PRIVATE, true, Payload, User, User, m_Server, Whisper );
				...
				{
					if( Payload.isEmpty( ) )
						QueueChatCommand( m_GHost->GetLanguage( )->CurrentlyLoadedMapCFGIs( m_GHost->GetCurrentMap( )->GetCFGFile( ) ), User, Whisper );
					else
					{
						try
						{
							...
							if( !MapCFGPath.exists() )
							{
								...
								QueueChatCommand( m_GHost->GetLanguage( )->ErrorListingMapConfigs( ), User, Whisper );
							}
							else
							{
								...
								if( Matches == 0 )
									QueueChatCommand( m_GHost->GetLanguage( )->NoMapConfigsFound( ), User, Whisper );
				...
				if( Command == "loadsg" && !Payload.isEmpty( ) )
				{
					...
					if( Payload.indexOf( "/" ) != -1 || Payload.indexOf( "\\" ) != -1 )
						QueueChatCommand( m_GHost->GetLanguage( )->UnableToLoadSaveGamesOutside( ), User, Whisper );
					else
					{
						...
						{
							if( m_GHost->GetCurrentGame( ) )
								QueueChatCommand( m_GHost->GetLanguage( )->UnableToLoadSaveGameGameInLobby( ), User, Whisper );
							else
							{
							...
							QueueChatCommand( m_GHost->GetLanguage( )->UnableToLoadSaveGameDoesntExist( File ), User, Whisper );
					}
				}
					...
					else
					{
						...
						try
						{
							...
							if( !MapPath.exists() )
							{
								...
								QueueChatCommand( m_GHost->GetLanguage( )->ErrorListingMaps( ), User, Whisper );
							}
							else
							{
								...
								if( Matches == 0 )
									QueueChatCommand( m_GHost->GetLanguage( )->NoMapsFound( ), User, Whisper );
								else if (files.contains(Pattern))
								{
									QueueChatCommand( m_GHost->GetLanguage( )->LoadingConfigFile( Pattern ), User, Whisper );
								...
								{
									QString File = files.at(0);
									QueueChatCommand( m_GHost->GetLanguage( )->LoadingConfigFile( File ), User, Whisper );
				...
				if( Command == "open" && !Payload.isEmpty( ) && m_GHost->GetCurrentGame( ) )
				{
					if( !m_GHost->GetCurrentGame( )->GetLocked( ) )
					{
						...
						while( !SS.atEnd( ) )
						{
							...
							if( SS.status() != QTextStream::Ok )
							{
								...
								m_GHost->GetCurrentGame( )->OpenSlot( (unsigned char)( SID - 1 ), true );
						}
					}
					else
						QueueChatCommand( m_GHost->GetLanguage( )->TheGameIsLockedBNET( ), User, Whisper );
				...
				if( Command == "openall" && m_GHost->GetCurrentGame( ) )
				{
					if( !m_GHost->GetCurrentGame( )->GetLocked( ) )
						m_GHost->GetCurrentGame( )->OpenAllSlots( );
					else
						QueueChatCommand( m_GHost->GetLanguage( )->TheGameIsLockedBNET( ), User, Whisper );
				}
				...
				if( Command == "priv" && !Payload.isEmpty( ) )
					m_GHost->CreateGame( m_GHost->GetCurrentMap( ), GAME_PRIVATE, false, Payload, User, User, m_Server, Whisper );
				...
				if( Command == "privby" && !Payload.isEmpty( ) )
				{
					...
					if( GameNameStart != -1 )
					{
						...
						m_GHost->CreateGame( m_GHost->GetCurrentMap( ), GAME_PRIVATE, false, GameName, Owner, User, m_Server, Whisper );
					}
				}
				...
				if( Command == "pub" && !Payload.isEmpty( ) )
					m_GHost->CreateGame( m_GHost->GetCurrentMap( ), GAME_PUBLIC, false, Payload, User, User, m_Server, Whisper );
				...
				if( Command == "pubby" && !Payload.isEmpty( ) )
				{
					...
					if( GameNameStart != -1 )
					{
						Owner = Payload.mid( 0, GameNameStart );
						GameName = Payload.mid( GameNameStart + 1 );
						m_GHost->CreateGame( m_GHost->GetCurrentMap( ), GAME_PUBLIC, false, GameName, Owner, User, m_Server, Whisper );
					}
				}
				...
				if( Command == "reload" )
				{
					if( IsRootAdmin( User ) )
					{
						QueueChatCommand( m_GHost->GetLanguage( )->ReloadingConfigurationFiles( ), User, Whisper );
						...
						else
						{
							...
							else
							{
								...
								if( GameNumber - 1 < m_GHost->m_Games.size( ) )
									...
									QueueChatCommand( m_GHost->GetLanguage( )->GameNumberDoesntExist( QString::number( GameNumber ) ), User, Whisper );
							}
						}
					}
					else
				...
				if( Command == "sp" && m_GHost->GetCurrentGame( ) && !m_GHost->GetCurrentGame( )->GetCountDownStarted( ) )
				{
					if( !m_GHost->GetCurrentGame( )->GetLocked( ) )
					{
						m_GHost->GetCurrentGame( )->SendAllChat( m_GHost->GetLanguage( )->ShufflingPlayers( ) );
						m_GHost->GetCurrentGame( )->ShuffleSlots( );
					}
					else
						QueueChatCommand( m_GHost->GetLanguage( )->TheGameIsLockedBNET( ), User, Whisper );
				}
				...
				if( Command == "start" && m_GHost->GetCurrentGame( ) && !m_GHost->GetCurrentGame( )->GetCountDownStarted( ) && m_GHost->GetCurrentGame( )->GetNumHumanPlayers( ) > 0 )
				{
					if( !m_GHost->GetCurrentGame( )->GetLocked( ) )
					{
						...
						if( Payload == "force" )
							m_GHost->GetCurrentGame( )->StartCountDown( true );
						else
							m_GHost->GetCurrentGame( )->StartCountDown( false );
					}
					else
						QueueChatCommand( m_GHost->GetLanguage( )->TheGameIsLockedBNET( ), User, Whisper );
				}
				...
				if( Command == "swap" && !Payload.isEmpty( ) && m_GHost->GetCurrentGame( ) )
				{
					if( !m_GHost->GetCurrentGame( )->GetLocked( ) )
					{
						quint32 SID1;
						...
						else
						{
							...
							else
							{
								...
								if( SS.status() != QTextStream::Ok )
									...
									m_GHost->GetCurrentGame( )->SwapSlots( (unsigned char)( SID1 - 1 ), (unsigned char)( SID2 - 1 ) );
							}
						}
					}
					else
				...
				if( Command == "unhost" )
				{
					if( m_GHost->GetCurrentGame( ) )
					{
						if( m_GHost->GetCurrentGame( )->GetCountDownStarted( ) )
							QueueChatCommand( m_GHost->GetLanguage( )->UnableToUnhostGameCountdownStarted( m_GHost->GetCurrentGame( )->GetDescription( ) ), User, Whisper );

						// if the game owner is still in the game only allow the root admin to unhost the game

						else if( m_GHost->GetCurrentGame( )->GetPlayerFromName( m_GHost->GetCurrentGame( )->GetOwnerName( ), false ) && !IsRootAdmin( User ) )
							QueueChatCommand( m_GHost->GetLanguage( )->CantUnhostGameOwnerIsPresent( m_GHost->GetCurrentGame( )->GetOwnerName( ) ), User, Whisper );
						else
						{
							QueueChatCommand( m_GHost->GetLanguage( )->UnhostingGame( m_GHost->GetCurrentGame( )->GetDescription( ) ), User, Whisper );
							m_GHost->GetCurrentGame( )->deleteLater();
						}
					}
					else
						QueueChatCommand( m_GHost->GetLanguage( )->UnableToUnhostGameNoGameInLobby( ), User, Whisper );
				}
	...
	else if( Event == CBNETProtocol :: EID_INFO )
	{
		...
		if( m_GHost->GetCurrentGame( ) && m_GHost->GetCurrentGame( )->GetPlayerFromName( UserName, true ) )
		{
			if( Message.indexOf( "is away" ) != -1 )
				m_GHost->GetCurrentGame( )->SendAllChat( m_GHost->GetLanguage( )->SpoofPossibleIsAway( UserName ) );
			else if( Message.indexOf( "is unavailable" ) != -1 )
				m_GHost->GetCurrentGame( )->SendAllChat( m_GHost->GetLanguage( )->SpoofPossibleIsUnavailable( UserName ) );
			else if( Message.indexOf( "is refusing messages" ) != -1 )
				m_GHost->GetCurrentGame( )->SendAllChat( m_GHost->GetLanguage( )->SpoofPossibleIsRefusingMessages( UserName ) );
				...
				m_GHost->GetCurrentGame( )->SendAllChat( m_GHost->GetLanguage( )->SpoofDetectedIsInPrivateChannel( UserName ) );
			...
			if( Message.indexOf( "is using Warcraft III The Frozen Throne in game" ) != -1 || Message.indexOf( "is using Warcraft III Frozen Throne and is currently in  game" ) != -1 )
			{
				...
				if( Message.indexOf( m_GHost->GetCurrentGame( )->GetGameName( ) ) != -1 || Message.indexOf( m_GHost->GetCurrentGame( )->GetLastGameName( ) ) != -1 )
					m_GHost->GetCurrentGame( )->AddToSpoofed( m_Server, UserName, false );
				else
					m_GHost->GetCurrentGame( )->SendAllChat( m_GHost->GetLanguage( )->SpoofDetectedIsInAnotherGame( UserName ) );
			}
		}
	}
